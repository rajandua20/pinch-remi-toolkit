...
function maxRefundCentsFromEnv() {
 ...
 */
export function buildServer(configOverride) {
    let client = null;
    const getClient = () => (client ??= new PinchClient(configOverride ?? configFromEnv()));
    const server = new McpServer(SERVER_INFO, {
        instructions: "Tools for the Pinch Payments API (Australian payments: direct debit, cards, payment links, subscriptions). " +
...
function checkRateLimit(key) {
    ...
    if (rateBuckets.size > RATE_BUCKET_CAP) {
        for (const [k, b] of rateBuckets)
            if (now - b.last > 120_000)
                rateBuckets.delete(k);
        if (rateBuckets.size > RATE_BUCKET_CAP)
            rateBuckets.clear(); // last resort
    }
    ...
    bucket.tokens = Math.min(RATE_LIMIT_PER_MIN, bucket.tokens + ((now - bucket.last) / 60_000) * RATE_LIMIT_PER_MIN);
    bucket.last = now;
    if (bucket.tokens >= 1) {
        bucket.tokens -= 1;
...
function jsonError(res, status, message, extraHeaders) {
    res.writeHead(status, { "Content-Type": "application/json", ...extraHeaders });
    res.end(JSON.stringify({ jsonrpc: "2.0", error: { code: -32000, message }, id: null }));
}
function applyCorsHeaders(res, opts) {
    if (!opts.cors)
        return;
...
function startHttp(port, opts) {
    ...
    const httpServer = http.createServer(async (req, res) => {
        ...
        if (req.method === "GET" && url.pathname === "/meta") {
            const usingHeaders = Boolean(headerValue(req, "x-pinch-merchant-id") && headerValue(req, "x-pinch-secret-key"));
            res.writeHead(200, { "Content-Type": "application/json" });
    ...
    httpServer.listen(port, opts.bind, () => {
        ...
        console.error(`[pinch-mcp] Streamable HTTP listening on http://${opts.bind}:${port}/mcp (stateless` +
...
async function main() {
    ...
    if (httpFlag !== -1) {
        ...
        if (!Number.isFinite(port) || port <= 0 || port > 65535) {
            console.error("Usage: pinch-mcp --http <port> [--cors] [--cors-origin <origin>] [--bind <addr>] [--allow-live]");
            process.exit(1);
        }
    ...
    console.error(`[pinch-mcp] stdio transport connected (env: ${process.env.PINCH_ENV ?? "test"}${process.env.PINCH_MERCHANT_ID ? "" : ", NO CREDENTIALS — tools will error until set"})`);
}
main().catch((err) => {
