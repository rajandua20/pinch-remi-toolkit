...
import { z } from "zod";
import { PinchApiError, formatAud, centsToAud } from "./pinch-client.js";
...
function previewResult(wouldDo, params) {
    return jsonResult({
        preview: true,
        wouldDo,
        params,
 ...
 */
function extractDishonour(payment) {
...
export function newDetailBudget(max = 10) {
 ...
 */
async function resolveDishonour(client, payment, budget) {
    ...
    budget.remaining--;
    try {
        const detail = await client.get(`/payments/${encodeURIComponent(payment.id)}`);
...
function extractPayerId(payment) {
 ...
 */
export function annotateDishonouredPayment(payment, resolved) {
...
export async function computeCashflowSummary(client, days) {
    const [processed, scheduled] = await Promise.all([
        client.getAllPages("/payments/processed", { startDate: todayPlus(-days) }),
        client.getAllPages("/payments/scheduled"),
    ]);
    const cents = (p) => (typeof p.amount === "number" ? p.amount : 0);
    const payerName = (p) => p.payer?.fullName ??
    ...
    const sumCents = (list) => list.reduce((s, p) => s + cents(p), 0);
 ...
 */
function allocateByPercent(totalCents, percents) {
...
function parseSplitMeta(metadata) {
    ...
    for (const pair of metadata.split(";")) {
        ...
        if (idx > 0)
            kv.set(pair.slice(0, idx).trim(), pair.slice(idx + 1).trim());
    }
    if (!kv.has("split"))
        return undefined;
    return {
...
async function ensurePayer(client, email, name) {
    const { items } = await client.getAllPages("/payers", { filter: email });
    const match = items.find((p) => (p.emailAddress ?? "").toLowerCase() === email.toLowerCase());
    if (match)
        return { payer: match, created: false };
    const parts = (name ?? "").trim().split(/\s+/).filter(Boolean);
    const created = await client.post("/payers", {
...
function estFeeCents(amountCents) {
    return Math.round(amountCents * EST_CARD_FEE_RATE);
}
/** Money block for one collection event (per charge). */
function moneyBlock(amountCents, platformFeePercent) {
    const fee = estFeeCents(amountCents);
    const platform = platformFeePercent !== undefined ? Math.round((amountCents * platformFeePercent) / 100) : null;
    ...
    switch (c.kind) {
        ...
        case "split": {
            if (!c.parties || c.parties.length === 0)
                return null; // event-driven split is fine
            if (c.parties.length < 2)
                return `${c.name}: split needs ≥2 parties (or omit parties for event-driven)`;
            const withAmt = c.parties.filter((p) => p.amountCents !== undefined);
            const withPct = c.parties.filter((p) => p.sharePercent !== undefined);
            if (withAmt.length > 0 && withPct.length > 0)
                return `${c.name}: split parties must ALL use amountCents or ALL use sharePercent`;
            if (withAmt.length + withPct.length < c.parties.length)
                return `${c.name}: every split party needs amountCents or sharePercent`;
            if (withPct.length === c.parties.length) {
                const sum = c.parties.reduce((s, p) => s + (p.sharePercent ?? 0), 0);
                if (Math.abs(sum - 100) > 0.01)
                    return `${c.name}: split sharePercent must sum to 100 (got ${sum})`;
            }
            else {
                const sum = c.parties.reduce((s, p) => s + (p.amountCents ?? 0), 0);
                if (sum !== c.amountCents)
                    return `${c.name}: split party amounts (${sum}c) must sum to amountCents (${c.amountCents}c)`;
            }
            return null;
        }
    }
 ...
 */
function compileBillingComponent(c, startDate, horizonDate, platformFeePercent) {
    const flags = [];
    if (platformFeePercent !== undefined) {
        flags.push(`${platformFeePercent}% platform fee NOT applied automatically — see needsReview`);
    }
    const events = [];
    const ev = (date, description, amountCents) => {
        if (date <= horizonDate) {
    ...
    let mapsTo = "";
    let scheduleSummary = "";
    let provisioning = "event-driven";
    let exampleCall;
    const firstEvents = [];
    const pushFirst = (date, amountCents, description) => {
        if (firstEvents.length < 4)
            firstEvents.push({ date, amountAud: formatAud(amountCents), description });
    };
    const recurringSeries = (first, count, label) => {
        ...
        for (let k = 0;; k++) {
            ...
            if (k < 4)
                pushFirst(d, c.amountCents, label);
            if (d > horizonDate && k >= 4)
                break;
            ev(d, label, c.amountCents);
            if (count === null && d > horizonDate)
                break;
        }
    };
    switch (c.kind) {
...
function planMatchesSpec(plan, spec) {
    const rp = plan.recurringPayment;
    const want = spec.recurringPayment;
    if (plan.name !== spec.name || rp == null)
        return false;
    if ((plan.metadata ?? "") !== (spec.metadata ?? ""))
        return false;
    const eq = (a, b) => (a ?? null) === (b ?? null);
    if (rp.amountInCents !== want.amountInCents ||
    ...
    const fixed = Array.isArray(plan.fixedPayments) ? plan.fixedPayments : [];
    const wantFixed = spec.fixedPayments ?? [];
    if (fixed.length !== wantFixed.length)
        return false;
    for (let i = 0; i < wantFixed.length; i++) {
        if (fixed[i].amountInCents !== wantFixed[i].amountInCents ||
...
function addIntervals(isoDateStr, interval, k) {
    if (interval === "monthly") {
 ...
 */
async function importPayerBatch(client, payers) {
    const ok = [];
    const failed = [];
    for (const p of payers) {
        try {
            const { items } = await client.getAllPages("/payers", { filter: p.email });
 ...
 */
function buildComponentPlanSpec(c, anchor, metadata) {
    ...
    const spec = {
        ...
        recurringPayment: {
            ...
            ...(dep !== undefined ? { startDateOffset: daysFromTo(subStart, firstRecurring), startDateInterval: "days" } : {}),
        ...
        ...(dep !== undefined
            ? { fixedPayments: [{ amountInCents: dep, description: `${c.name} — deposit`, scheduledDateOffset: 0, scheduledDateInterval: "days", cancelPlanOnFailure: false }] }
            : {}),
    };
...
async function ensurePlan(client, spec) {
    const { items: plans } = await client.getAllPages("/plans");
    return plans.find((p) => planMatchesSpec(p, spec)) ?? (await client.post("/plans", spec));
}
/**
 ...
 */
async function subscribeOrSetup(client, payerId, planId, subStart, firstChargeCents, setupDescription) {
    const detail = await client.get(`/payers/${encodeURIComponent(payerId)}`);
    const sources = Array.isArray(detail.sources) ? detail.sources : [];
    if (sources.length === 0) {
        const link = await client.post("/payment-links", {
            payerId,
    ...
    const sub = await client.post("/subscriptions", { planId, payerId, startDate: subStart });
 ...
 */
async function provisionBlueprintComponents(client, components, compiled, anchor, metadata) {
    const provisioned = [];
    for (let i = 0; i < components.length; i++) {
        ...
        try {
            if (c.kind === "split") {
                ...
                const r = await subscribeOrSetup(client, payer.id, plan.id, subStart, firstChargeCents, `${c.name} — first payment & payment method setup`);
                if (r.pendingSetup) {
                    provisioned.push({
...
function isRecurringComponent(c) {
    return ((c.kind === "membership" || c.kind === "term" || (c.kind === "package" && c.termPayments !== undefined)) &&
        c.interval !== undefined);
}
/** Register every pinch tool on the given McpServer. Returns names + manifest hash. */
export function registerPinchTools(server, options) {
    ...
    const audited = (name, fn) => {
        return async (...args) => {
            ...
            try {
                ...
                console.error(`[pinch-mcp] audit tool=${name} outcome=${outcome} ms=${Date.now() - started} tenant=${tenantTag()}`);
            }
        };
    };
    ...
    const reg = (name, config, cb) => {
        ...
        return server.registerTool(name, config, audited(name, cb));
    };
    // =========================================================================
    // READ TOOLS (no side effects)
    // =========================================================================
    reg(track("pinch_health"), {
        title: "Pinch health check",
        description: "Verify Pinch credentials and connectivity: fetches an OAuth token and performs a trivial GET " +
        ...
        return jsonResult({
            ...health,
            note: client.env === "test"
                ? "Sandbox (test) environment — no real money moves."
                : "LIVE environment — real money. Write tools will move real funds.",
        });
    }));
    reg(track("pinch_list_payments"), {
        title: "List payments",
        description: "List Pinch payments. By default lists PROCESSED payments (things that have run), optionally filtered by " +
        ...
        inputSchema: {
            status: z
                .string()
                .optional()
                .describe("Filter by payment status, e.g. settled | approved | dishonoured | processing | scheduled | cancelled"),
        ...
        const { items, totalItems, truncated } = await client.getAllPages(path, query);
        let payments = items;
        if (status)
            payments = payments.filter((p) => p.status === status);
        if (scheduled && (from || to)) {
            // Client-side date filter for the scheduled list.
            payments = payments.filter((p) => (!from || pinchDate(p.transactionDate) >= from) &&
    ...
    reg(track("pinch_get_payment"), {
        title: "Get payment",
        description: "Fetch one Pinch payment by id (pmt_...), including its attempts and any dishonour detail. If the payment " +
        ...
        const client = getClient();
        const payment = await client.get(`/payments/${encodeURIComponent(paymentId)}`);
    ...
    reg(track("pinch_list_failed_payments"), {
        title: "List failed (dishonoured) payments",
        description: "List payments that FAILED (status 'dishonoured'), each annotated with a diagnosis from the dishonour " +
        ...
        const client = getClient();
        const { items, truncated } = await client.getAllPages("/payments/processed", {
            startDate: from,
            endDate: to,
        });
        ...
        const atRiskCents = failed.reduce((sum, p) => sum + (typeof p.amount === "number" ? p.amount : 0), 0);
        return jsonResult({
    ...
    reg(track("pinch_get_payer"), {
        title: "Get payer",
        description: "Fetch one Pinch payer by id (pyr_...), including contact details and stored payment sources " +
        ...
        const client = getClient();
        const payer = await client.get(`/payers/${encodeURIComponent(payerId)}`);
        return jsonResult({ payer });
    }));
    reg(track("pinch_list_payers"), {
        title: "List payers",
        description: "List Pinch payers (customers), optionally filtered with a free-text search (matched server-side against " +
        ...
        const client = getClient();
        const { items, totalItems, truncated } = await client.getAllPages("/payers", {
            filter: search,
        });
        return jsonResult({
            count: items.length,
    ...
    reg(track("pinch_list_subscriptions"), {
        title: "List subscriptions (with stall detection)",
        description: "List Pinch subscriptions, cross-referenced against the last ~90 days of processed payments to flag " +
        ...
        const [subsResult, paymentsResult] = await Promise.all([
            client.getAllPages("/subscriptions"),
            client.getAllPages("/payments/processed", { startDate: todayPlus(-90) }),
        ]);
        ...
        for (const p of paymentsResult.items) {
            ...
            if (subId)
                (bySub.get(subId) ?? bySub.set(subId, []).get(subId)).push(p);
            const payerId = extractPayerId(p);
            if (payerId)
                (byPayer.get(payerId) ?? byPayer.set(payerId, []).get(payerId)).push(p);
        }
        const STALL_DAYS = 35;
        const RECENT_DISHONOUR_DAYS = 14;
        const subscriptions = subsResult.items.map((sub) => {
            const related = bySub.get(sub.id) ?? byPayer.get(sub.payer?.id ?? sub.payerId ?? "") ?? [];
            let lastSuccessDate;
            let recentDishonours = 0;
            for (const p of related) {
                const when = p.transactionDate;
                if (SUCCESS_STATUSES.has(p.status)) {
                    if (!lastSuccessDate || (when ?? "") > lastSuccessDate)
                        lastSuccessDate = when;
                }
                else if (p.status === "dishonoured") {
                    const age = daysAgo(when);
                    if (age !== undefined && age <= RECENT_DISHONOUR_DAYS)
                        recentDishonours++;
                }
            }
        ...
        return jsonResult({
            ...
            note: "Stall detection uses the last 90 days of processed payments (max 5 pages); " +
                "'stalled' = active subscription with no success in 35+ days or a dishonour in the last 14 days.",
            stalledSubscriptions: stalled,
            subscriptions,
        });
    }));
    reg(track("pinch_get_subscription"), {
        title: "Get subscription",
        description: "Fetch one Pinch subscription by id (sub_...): plan, payer, status (active | cancelled | complete), " +
        ...
        const client = getClient();
        const subscription = await client.get(`/subscriptions/${encodeURIComponent(subscriptionId)}`);
        return jsonResult({ subscription });
    }));
    reg(track("pinch_list_events"), {
        title: "List events",
        description: "Poll the Pinch event feed (GET /events), optionally filtered by eventType and start date. Useful types: " +
        ...
        const client = getClient();
        const { items, totalItems, truncated } = await client.getAllPages("/events", {
            eventType,
            startDate: from,
        });
        return jsonResult({ count: items.length, totalItems, truncated, events: items });
    }));
    reg(track("pinch_cashflow_summary"), {
        title: "Cashflow summary",
        description: "One-call merchant cashflow snapshot — answers 'how am I doing?'. Over the last N days (default 7, max 90): " +
    ...
    reg(track("pinch_payer_statement"), {
        title: "Payer statement of account",
        description: "Build a statement of account for one payer: every processed payment in the lookback window (default 90 " +
        ...
        let payer;
        if (payerId) {
            payer = await client.get(`/payers/${encodeURIComponent(payerId)}`);
        }
        else if (payerEmail) {
            const { items } = await client.getAllPages("/payers", { filter: payerEmail });
        ...
        const { items: payments, truncated } = await client.getAllPages(`/payments/payer/${encodeURIComponent(resolvedId)}`);
        const cutoff = todayPlus(-periodDays);
        const today = todayPlus(0);
        const dateOf = (p) => pinchDate(p.transactionDate);
        const cents = (p) => (typeof p.amount === "number" ? p.amount : 0);
        ...
        for (const p of inScope) {
            const diagnosis = p.status === "dishonoured"
        ...
        for (const p of inScope) {
            if (SUCCESS_STATUSES.has(p.status))
                collectedCents += cents(p);
            else if (p.status === "dishonoured")
                failedCents += cents(p);
            else if (p.status === "scheduled" && dateOf(p) >= today)
                upcomingScheduledCents += cents(p);
        }
        ...
        for (const line of lines) {
            txt.push(`${line.date.padEnd(12)}${line.status.padEnd(14)}${line.amountAud.padStart(10)}  ${line.description}`);
            if (line.dishonourReason)
                txt.push(`${" ".repeat(12)}! ${line.dishonourReason}`);
        }
        if (lines.length === 0)
            txt.push("(no payments in this period)");
        txt.push("-".repeat(72));
    ...
    reg(track("pinch_get_split_status"), {
        title: "Split payment status",
        description: "Track a split bill created with pinch_create_split: who has paid, who is outstanding, who failed " +
        ...
        const linksResult = await client.getAllPages("/payment-links");
        const splitLinks = linksResult.items.filter((l) => typeof l.metadata === "string" && l.metadata.includes(splitId));
        ...
        const paymentsResult = await client.getAllPages("/payments/processed", {
            startDate: todayPlus(-180),
        });
        const splitPayments = paymentsResult.items.filter((p) => typeof p.metadata === "string" && p.metadata.includes(splitId));
        const truncated = linksResult.truncated || paymentsResult.truncated;
        if (splitLinks.length === 0 && splitPayments.length === 0) {
            return errorResult(`No payment links or payments found for split ${splitId}` +
                (truncated ? " within the scanned pages (5-page cap — the split may be older)." : "."), { splitId, truncated });
        }
        // Build one party record per link (or per orphan payment as fallback).
        const today = todayPlus(0);
        const daysBetween = (fromDate, toDate) => Math.max(0, Math.round((Date.parse(toDate) - Date.parse(fromDate)) / 86_400_000));
        let description = "";
        let totalCents = 0;
        let dueDate = null;
        const detailBudget = newDetailBudget();
        ...
        for (const link of splitLinks) {
            ...
            if (typeof meta.totalCents === "number")
                totalCents = meta.totalCents;
            if (meta.dueDate)
                dueDate = meta.dueDate;
            const amountCents = typeof link.amountInCents === "number" ? link.amountInCents : (meta.amountCents ?? 0);
            const payerId = link.payer?.id ?? null;
            const name = link.payer?.fullName ??
            ...
            const nested = Array.isArray(link.payments) ? link.payments : [];
            const matched = splitPayments.filter((p) => extractPayerId(p) === payerId && centsOf(p) === amountCents);
            ...
            if (success) {
                ...
                status = "failed";
                const diagnosis = diagnoseDishonour((await resolveDishonour(client, dishonoured, detailBudget))?.type);
                dishonourOut = {
                    code: diagnosis.code,
                    plainEnglish: diagnosis.plainEnglish,
        ...
        for (const p of splitPayments) {
            ...
            if (coveredPayers.has(payerId))
                continue;
            coveredPayers.add(payerId);
            const meta = parseSplitMeta(p.metadata) ?? {};
            if (typeof meta.totalCents === "number")
                totalCents = totalCents || meta.totalCents;
        ...
        const largest = outstanding.reduce((best, p) => (best === null || p.amountCents > best.amountCents ? p : best), null);
        let riskNote;
        if (outstanding.length === 0) {
            riskNote = "Fully collected — no outstanding exposure on this split.";
        }
        else if (largest) {
            const ageText = largest.daysOutstanding !== null ? ` for ${largest.daysOutstanding} days` : "";
            riskNote =
        ...
        const suggestedActions = outstanding.map((p) => p.status === "failed" && p.dishonour
    ...
    reg(track("pinch_settlement_summary"), {
        title: "Settlement summary (see the money)",
        description: "Reconciliation-lite: what Pinch has transferred to the merchant's bank and what made each transfer up. " +
        ...
        const [transfersResult, processedResult] = await Promise.all([
            client.getAllPages("/transfers"),
            client.getAllPages("/payments/processed", { startDate: cutoff }),
        ]);
        ...
        const nameOf = (p) => p?.payer?.fullName ??
            ([p?.payer?.firstName, p?.payer?.lastName].filter(Boolean).join(" ") || undefined);
        ...
        for (let i = 0; i < inWindow.length; i++) {
            ...
            if (i < DETAIL_CAP && t.id) {
                try {
                    const { items: lines } = await client.getAllPages(`/transfers/items/${encodeURIComponent(t.id)}`);
        ...
        const hasTransferred = (p) => {
            if (p.actualTransferDate)
                return true;
            const attempts = Array.isArray(p.attempts) ? p.attempts : [];
            return attempts.some((a) => a.actualTransferDate);
        };
        const unsettledPayments = processedResult.items.filter((p) => SUCCESS_STATUSES.has(p.status) && !hasTransferred(p));
        const unsettledCents = unsettledPayments.reduce((s, p) => s + (typeof p.amount === "number" ? p.amount : 0), 0);
        return jsonResult({
            periodDays,
            transfers,
            unsettled: {
            ...
            detailTruncated,
            generatedAt: new Date().toISOString(),
            basisNote: "Composed from GET /transfers (net per transfer) + GET /transfers/items/{id} line items (Settlement/" +
    ...
    const confirmParam = z
        .boolean()
        .optional()
        .describe("Must be exactly true to execute. Anything else returns a preview of what WOULD happen, for human approval.");
    ...
    reg(track("pinch_create_payment_link"), {
        title: "Create payment link (guarded)",
        description: "Create a Pinch hosted payment link (checkout page) for a payer — the standard way to (re-)collect payment " +
        ...
        inputSchema: {
            ...
            allowedPaymentMethods: z
                ...
                .min(1)
                .optional()
                .describe('Payment methods offered on the checkout page. REQUIRED by the Pinch API — defaults to ["credit-card","bank-account"].'),
            metadata: z
                .string()
                .optional()
        ...
        if (confirm !== true) {
            return previewResult(`Create a Pinch hosted payment link for ${formatAud(amountCents)} (“${description}”) addressed to ` +
        ...
        return jsonResult({
            ...
            allowedPaymentMethods: methods,
            metadata: metadata ?? null,
            note: "Send this URL to the customer — paying it also stores their new payment method. On completion they are " +
                "redirected to returnUrl with ?paymentLinkId=&paymentId= appended; any metadata is copied onto the Payment.",
        });
    }));
    reg(track("pinch_retry_payment"), {
        title: "Retry a dishonoured payment (guarded)",
        description: "Retry a FAILED payment by creating a new scheduled payment cloned from the dishonoured original (Pinch's " +
        ...
        const original = await client.get(`/payments/${encodeURIComponent(paymentId)}`);
        if (original.status !== "dishonoured") {
            return errorResult(`Payment ${paymentId} has status "${original.status}" — only dishonoured payments can be retried.`);
        }
        ...
        let description = original.description ?? "Payment retry";
        if (client.env === "test")
            description = description.replace(/#[a-z-]+/gi, "").trim();
        description = `${description || "Payment retry"} (retry of ${paymentId})`.slice(0, 999);
        const hardWarning = diagnosis.retryable === "hard"
            ? `WARNING: "${diagnosis.code}" is a HARD failure — ${diagnosis.recommendedAction}`
            : undefined;
        if (confirm !== true) {
            return previewResult(`Create a new scheduled payment of ${formatAud(original.amount)} for payer ${payerIdOfOriginal} on ` +
        ...
        const retry = await client.post("/payments", {
            payerId: payerIdOfOriginal,
    ...
    reg(track("pinch_create_refund"), {
        title: "Create refund (guarded, capped)",
        description: "Refund a Pinch payment, fully (omit amountCents) or partially. Hard-capped by the PINCH_MAX_REFUND_CENTS " +
        ...
        if (refundCents === undefined) {
            const original = await getClient().get(`/payments/${encodeURIComponent(paymentId)}`);
        ...
        if (refundCents > maxRefundCents) {
            return errorResult(`Refund of ${formatAud(refundCents)} exceeds the PINCH_MAX_REFUND_CENTS cap of ${formatAud(maxRefundCents)}. ` +
                "Raise the cap via environment config or process this refund manually in the Pinch portal.", { requestedCents: refundCents, maxRefundCents });
        }
        const refundReason = reason ?? "Refund requested via pinch-mcp (human approved)";
        if (confirm !== true) {
            return previewResult(`Refund ${formatAud(refundCents)} of payment ${paymentId}` +
        ...
        const refund = await getClient().post("/refunds", {
            paymentId,
            amount: refundCents,
            reason: refundReason,
    ...
    reg(track("pinch_create_subscription"), {
        title: "Create subscription (guarded)",
        description: "Set up recurring billing: resolves (or creates) the payer, finds or creates a structurally matching Pinch " +
        ...
        inputSchema: {
            ...
            termPayments: z
                ...
                .max(104)
                .optional()
                .describe("End the subscription after exactly N recurring collections (e.g. 10 for a 10-week term). Mutually exclusive with endDate."),
            endDate: isoDate
                .optional()
            ...
            metadata: z
                .string()
                .optional()
                .describe("Free text (JSON preferred) stored on the Plan — Pinch adds it to EVERY payment generated from the plan. " +
                'Use for attribution, e.g. {"instructorId":"coach_42"}. Different metadata ⇒ a separate plan.'),
            confirm: confirmParam,
        },
    }, safe(async (args) => {
        const { payerEmail, payerId, payerName, amountCents, interval, description, startDate, termPayments, endDate, depositCents, depositDate, metadata, confirm, } = args;
        if (!payerEmail && !payerId) {
        ...
        if (termPayments !== undefined) {
            ...
            let n = 1;
            while (daysFromTo(addIntervals(start, interval, n), endDate) >= 0)
                n++;
            paymentsCount = n;
            lastPaymentDate = addIntervals(start, interval, n - 1);
            endRule = `ends by ${endDate} (${n} payments, last on ${lastPaymentDate})`;
        }
        const termTotalCents = paymentsCount !== null ? (depositCents ?? 0) + paymentsCount * amountCents : null;
        const schedule = {
            ...
            recurring: {
                ...
                payments: paymentsCount ?? "ongoing",
                lastPaymentDate,
                endRule,
            },
            termTotalCents,
        ...
        const planName = `${description} (${interval} ${formatAud(amountCents)}` +
        ...
        const planSpec = {
            ...
            recurringPayment: {
                ...
                ...(recurringOffsetDays > 0
                    ? { startDateOffset: recurringOffsetDays, startDateInterval: "days" }
                    : {}),
                ...
                endType: termPayments !== undefined ? "number-of-payments" : endDate !== undefined ? "end-date" : "never",
                ...(termPayments !== undefined ? { endAfterNumberOfPayments: termPayments } : {}),
                ...(endDate !== undefined
                    ? { endDateOffset: daysFromTo(subStart, endDate), endDateInterval: "days" }
                    : {}),
                cancelPlanOnFailure: false,
            },
        ...
        let existingPayer;
        if (payerId) {
            existingPayer = await client.get(`/payers/${encodeURIComponent(payerId)}`);
        }
        else if (payerEmail) {
            const { items } = await client.getAllPages("/payers", { filter: payerEmail });
            existingPayer = items.find((p) => (p.emailAddress ?? "").toLowerCase() === payerEmail.toLowerCase());
        }
        const { items: plans } = await client.getAllPages("/plans");
        ...
        const planSummary = existingPlan
            ...
            : "would create";
        if (confirm !== true) {
            const scheduleWords = (schedule.deposit ? `${schedule.deposit.amountAud} deposit on ${schedule.deposit.date}, then ` : "") +
            ...
            return jsonResult({
                preview: true,
                wouldDo: `Create a subscription (“${description}”) for ` +
        ...
        if (!resolvedPayer) {
            ...
            resolvedPayer = await client.post("/payers", {
        ...
        if (!plan) {
            ...
            plan = await client.post("/plans", planSpec);
            planCreated = true;
        }
        ...
        const payerDetail = await client.get(`/payers/${encodeURIComponent(resolvedPayer.id)}`);
        const sources = Array.isArray(payerDetail.sources)
            ? payerDetail.sources
        ...
        if (sources.length === 0) {
            ...
            return jsonResult({
                ...
                note: "SUBSCRIPTION NOT CREATED YET — Pinch requires the payer to have a stored payment source before a " +
    ...
    reg(track("pinch_cancel_subscription"), {
        title: "Cancel subscription (guarded)",
        description: "Cancel a Pinch subscription (DELETE /subscriptions/{id}). Per the Pinch docs: payments already in " +
        ...
        const sub = await client.get(`/subscriptions/${encodeURIComponent(subscriptionId)}`);
        if (sub.status === "cancelled" || sub.status === "complete") {
            return errorResult(`Subscription ${subscriptionId} is already "${sub.status}" — nothing to cancel.`, { subscriptionId, status: sub.status });
        }
        const recurringCents = typeof sub.recurringPayment?.amount === "number"
        ...
        if (confirm !== true) {
            return jsonResult({
                preview: true,
                wouldDo: `Cancel subscription ${subscriptionId}` +
        ...
        try {
            const after = await client.get(`/subscriptions/${encodeURIComponent(subscriptionId)}`);
        ...
        return jsonResult({
            ...
            subscriptionId,
            status: finalStatus,
            note: "Future scheduled payments have been deleted; any payment already in processing will still complete. " +
                "The payer's stored payment method was not removed.",
        });
    }));
    reg(track("pinch_create_split"), {
        title: "Create split payment (guarded)",
        description: "Split a shared bill (trainers splitting studio rent, coaches sharing a venue hire, clubs co-funding a " +
        ...
        if (withAmount.length > 0 && withPercent.length > 0) {
            return errorResult("Mixed share modes: use amountCents for ALL parts or sharePercent for ALL parts, not a mixture.");
        }
        let allocatedCents;
        let resolvedTotal;
        if (withPercent.length === parts.length) {
            ...
            if (totalCents !== undefined && totalCents !== sum) {
                return errorResult(`Explicit amounts sum to ${formatAud(sum)} but totalCents says ${formatAud(totalCents)} — they must match (or omit totalCents).`, { sumCents: sum, totalCents });
            }
            resolvedTotal = sum;
        ...
        if (confirm !== true) {
            return jsonResult({
                ...
                wouldDo: `Split “${description}” (${formatAud(resolvedTotal)} total) across ${parts.length} parties: create ` +
        ...
        return jsonResult({
            ...
            note: `Share each party their own link. Track collection anytime with pinch_get_split_status {splitId: "${splitId}"} — ` +
    ...
    const DESIGN_BILLING_DESC = "Deterministic billing-blueprint compiler: maps structured components (membership, term, package, " +
    ...
    reg(track("pinch_design_billing"), {
        ...
        const errors = components
            .map((c) => validateDesignComponent(c))
            .filter((e) => e !== null);
        if (errors.length > 0)
            return errorResult("Blueprint validation failed.", { errors });
        ...
        const compiled = components.map((c) => compileBillingComponent(c, anchor, horizon, platformFeePercent));
        ...
        try {
            const { items: plans } = await getClient().getAllPages("/plans");
            for (let i = 0; i < components.length; i++) {
                const c = components[i];
                if ((c.kind === "membership" || c.kind === "term" || (c.kind === "package" && c.termPayments)) && c.interval) {
                    const hit = plans.find((p) => p.recurringPayment?.amountInCents === c.amountCents &&
                    ...
                    if (hit)
                        compiled[i].component.flags.push(`similar existing plan found (${hit.id} “${hit.name}”) — structural match decides reuse at provision time`);
                }
            }
        }
        catch {
        ...
        const platformFirstMonth = platformFeePercent !== undefined
        ...
        if (pausePolicy?.pausesPerYear !== undefined) {
            policyNotes.push(`Pause maps to cancel + re-subscribe (no native pause in Pinch): pinch_cancel_subscription on pause, ` +
        ...
        const needsReview = [];
        if (platformFeePercent !== undefined) {
            needsReview.push(`Platform fee retention (${platformFeePercent}%) requires Pinch Managed Merchants / application fees — ` +
        ...
        compiled.forEach((cc, i) => {
            ...
            const m = comp.money;
            txt.push(`   per charge: gross ${m.gross.aud} · est Pinch fee ${m.estPinchFee.aud}` +
        ...
        if (timeline.length > 0) {
            ...
            for (const e of timeline)
                txt.push(`   ${e.date}  ${e.amountAud.padStart(9)}  ${e.componentName} — ${e.description}`);
        }
        txt.push("");
        txt.push(`FIRST 30 DAYS: gross ${totals.firstMonthGrossAud} · est net ${totals.estFirstMonthNetAud}` +
            (totals.perInterval.length
                ? ` · recurring: ${totals.perInterval.map((p) => `${p.grossAud}/${p.interval}`).join(", ")}`
                : ""));
        for (const p of policyNotes)
            txt.push(`POLICY: ${p}`);
        for (const r of needsReview)
            txt.push(`NEEDS REVIEW: ${r}`);
        const blueprint = {
        ...
        if (confirm !== true) {
            return jsonResult({
                ...
                note: "Blueprint only — nothing was created. Re-call with confirm:true to provision the provisionable-now " +
        ...
        const provisioned = await provisionBlueprintComponents(getClient(), components, compiled, anchor, metadata);
        return jsonResult({
            blueprint,
            blueprintText: txt.join("\n"),
            provisioned,
    ...
    const IMPORT_PAYERS_DESC = "Bulk-import 1-50 payers (customers) into Pinch from {firstName, lastName, email, mobile?} records. " +
    ...
    reg(track("pinch_import_payers"), {
        ...
        if (confirm !== true) {
            return previewResult(`Import ${payers.length} payer record(s) into Pinch: each is matched by email (existing payers are ` +
    ...
    const ONBOARD_BUSINESS_DESC = "1-touch business setup: provisions a billing blueprint (same components as pinch_design_billing), " +
    ...
    reg(track("pinch_onboard_business"), {
        ...
        inputSchema: {
            ...
            assignPlan: z
                .string()
                .optional()
                .describe("Name of ONE recurring component (membership | term | package-with-instalments) to enrol every " +
        ...
        if (assignPlan !== undefined) {
            const recurring = components.filter(isRecurringComponent);
            enrolComponent = recurring.find((c) => c.name.toLowerCase() === assignPlan.toLowerCase());
            if (!enrolComponent) {
                return errorResult(`assignPlan "${assignPlan}" does not match a recurring component — enrolment needs a membership, ` +
                    "term, or package-with-instalments component.", { enrollableComponents: recurring.map((c) => c.name) });
            }
        }
        const anchor = startDate ?? todayPlus(1);
        const horizon = new Date(Date.parse(`${anchor}T00:00:00Z`) + 30 * 86_400_000)
            .toISOString()
            .slice(0, 10);
        const compiled = components.map((c) => compileBillingComponent(c, anchor, horizon));
        const payerList = payers ?? [];
        if (confirm !== true) {
            // Structured go-live plan — pure preview, zero API calls.
            return jsonResult({
                preview: true,
                wouldDo: `Onboard “${businessName}”: provision ${components.length} billing component(s), import ` +
                ...
                goLivePlan: {
                    ...
                    enrolment: enrolComponent
                        ? {
                            ...
                            perPayer: "one pendingSetup subscription + hosted setup link — paying the link collects the first " +
        ...
        const provisionedComponents = await provisionBlueprintComponents(client, components, compiled, anchor, metadata);
        ...
        if (enrolComponent && createdPayers.length > 0) {
            try {
                ...
                const plan = await ensurePlan(client, spec);
                const label = enrolComponent.depositCents !== undefined ? "deposit" : `first ${enrolComponent.interval} payment`;
                for (const p of createdPayers) {
                    try {
                        const r = await subscribeOrSetup(client, p.payerId, plan.id, subStart, firstChargeCents, `${enrolComponent.name} — ${label} & payment method setup`);
                        enrolments.push({
                            payerEmail: p.email,
                            payerId: p.payerId,
        ...
        if (setupLinks.length > 0) {
            nextSteps.push(`Send each of the ${setupLinks.length} payer(s) their hosted setup link — paying it collects the ` +
                "first charge AND stores their payment method.");
            nextSteps.push("After a payer's setup link is paid, re-call pinch_create_subscription with confirm:true for them — " +
                "the plan is already in place and will be reused; shift startDate so the collected first charge " +
                "isn't double-billed.");
        }
        nextSteps.push("Direct-debit collections run on Pinch's overnight processing cycle — first collections land the " +
            "business day after their scheduled date.");
        if (provisionedComponents.some((p) => p.action === "skipped")) {
            nextSteps.push("Event-driven components were not auto-provisioned — issue payment links per booking/sale " +
                "(see each component's exampleCall in pinch_design_billing).");
        }
        if (failedPayers.length > 0) {
            nextSteps.push(`${failedPayers.length} payer import(s) failed — fix the reported errors and re-run ` +
                "pinch_import_payers for just those payers (re-runs are safe: existing emails are reused).");
        }
        nextSteps.push("Once collections start, monitor pinch_cashflow_summary weekly and triage dishonours with " +
